function chebyshev_weights(f::Array{T,N},nodes::NTuple{N,Array{T,1}},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema(f::Array{T,N},nodes::NTuple{N,Array{T,1}},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended(f::Array{T,N},nodes::NTuple{N,Array{T,1}},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights(f::Array{T,N},poly::NTuple{N,Array{T,2}},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema(f::Array{T,N},poly::NTuple{N,Array{T,2}},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended(f::Array{T,N},poly::NTuple{N,Array{T,2}},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_threaded(f::Array{T,N},nodes::NTuple{N,Array{T,1}},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema_threaded(f::Array{T,N},nodes::NTuple{N,Array{T,1}},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended_threaded(f::Array{T,N},nodes::NTuple{N,Array{T,1}},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_threaded(f::Array{T,N},poly::NTuple{N,Array{T,2}},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema_threaded(f::Array{T,N},poly::NTuple{N,Array{T,2}},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended_threaded(f::Array{T,N},poly::NTuple{N,Array{T,2}},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights(f::Array{T,1},nodes::Array{T,1},order::S,domain=[one(T);-one(T)]) where {T<:AbstractFloat,S<:Integer}
...
function chebyshev_weights_extrema(f::Array{T,1},nodes::Array{T,1},order::S,domain=[one(T);-one(T)]) where {T<:AbstractFloat,S<:Integer}
...
function chebyshev_weights_extended(f::Array{T,1},nodes::Array{T,1},order::S,domain=[one(T);-one(T)]) where {T<:AbstractFloat,S<:Integer}
...
function chebyshev_weights(f::Array{T,1},poly::Array{T,2},order::S) where {T<:AbstractFloat,S<:Integer}
...
function chebyshev_weights_extrema(f::Array{T,1},poly::Array{T,2},order::S) where {T<:AbstractFloat,S<:Integer}
...
function chebyshev_weights_extended(f::Array{T,1},poly::Array{T,2},order::S) where {T<:AbstractFloat,S<:Integer}
...
function chebyshev_weights(f::Array{T,N},nodes::Array{Array{T,1},1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema(f::Array{T,N},nodes::Array{Array{T,1},1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended(f::Array{T,N},nodes::Array{Array{T,1},1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights(f::Array{T,N},poly::Array{Array{T,2},1},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema(f::Array{T,N},poly::Array{Array{T,2},1},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended(f::Array{T,N},poly::Array{Array{T,2},1},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_threaded(f::Array{T,N},nodes::Array{Array{T,1},1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema_threaded(f::Array{T,N},nodes::Array{Array{T,1},1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended_threaded(f::Array{T,N},nodes::Array{Array{T,1},1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_threaded(f::Array{T,N},poly::Array{Array{T,2},1},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extrema_threaded(f::Array{T,N},poly::Array{Array{T,2},1},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_weights_extended_threaded(f::Array{T,N},poly::Array{Array{T,2},1},order::NTuple{N,S}) where {T<:AbstractFloat,N,S<:Integer}
