...
function chebyshev_evaluate(weights::Array{T,N},x::Array{R,1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,R<:Number,N,S<:Integer}
...
function chebyshev_evaluate(weights::Array{T,N},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
