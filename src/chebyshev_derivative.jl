...
function chebyshev_derivative(weights::Array{T,N},x::Array{R,1},pos::S,order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,R<:Number,N,S<:Integer}
...
function chebyshev_gradient(weights::Array{T,N},x::Array{R,1},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,R<:Number,N,S<:Integer}
...
function chebyshev_derivative(weights::Array{T,N},pos::S,order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
...
function chebyshev_gradient(weights::Array{T,N},order::NTuple{N,S},domain=[ones(T,1,N);-ones(T,1,N)]) where {T<:AbstractFloat,N,S<:Integer}
